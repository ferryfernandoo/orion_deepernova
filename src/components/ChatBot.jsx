...
const ChatBot = () => {
  ...
  // ========== HANDLE SEND MESSAGE DENGAN SCROLL LUAS ==========
  const handleSendMessage = async (e) => {
    ...
    // ===== SCROLL YANG MENYISAKAN RUANG KOSONG LUAS =====
    setTimeout(() => {
      try {
        const scrollEl = document.querySelector('.messages-container');
        const msgEl = document.querySelector(`[data-msg-id="${userMessage.id}"]`);
        if (msgEl && scrollEl) {
          // Tambahkan class prefill-space (padding-bottom besar dari CSS)
          try { scrollEl.classList.add('prefill-space'); } catch (e) {}
          // Hitung posisi agar pesan user berada 100px di bawah batas atas container
          const rect = msgEl.getBoundingClientRect();
          const scrollTopTarget = scrollEl.scrollTop + rect.top - 100;
          scrollEl.scrollTo({ top: Math.max(0, scrollTopTarget), behavior: 'auto' });
        } else {
          scrollToBottom(true);
        ...
        <div className="modal-overlay" onClick={() => setShowPrivateModal(false)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <p>{userLanguage === 'id' ? 'Obrolan privat memungkinkan Anda untuk berbicara dengan Orion tanpa menyimpan riwayat percakapan. Percakapan ini tidak akan muncul di daftar riwayat chat Anda.' : 'Private chat allows you to talk with Orion without saving the conversation history. This chat will not appear in your chat history list.'}</p>
      ...
      <div className={`sidebar ${sidebarOpen ? 'open' : 'closed'}`}>
        <div className="sidebar-header">
          ...
          <div className="sidebar-header-actions">
            <button className="private-chat-btn" onClick={() => setShowPrivateModal(true)}>🔒</button>
        ...
        <div className="personality-section">
          <div className="personality-header"><span>🎭 AI Personality</span></div>
          <div className="personality-grid">
            {Object.values(PERSONALITIES).map((personality) => (
              <button key={personality.id} className={`personality-btn ${selectedPersonality === personality.id ? 'active' : ''}`} onClick={() => setSelectedPersonality(personality.id)} title={personality.description}>
                <span className="personality-emoji">{personality.emoji}</span>
                <span className="personality-name">{personality.name}</span>
              </button>
            ))}
          </div>
          <p className="personality-hint">{userLanguage === 'id' ? '💡 Ubah kepribadian AI untuk gaya percakapan yang berbeda' : '💡 Change AI personality for different conversation styles'}</p>
        </div>
