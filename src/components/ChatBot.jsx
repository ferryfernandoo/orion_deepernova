...
const ChatBot = () => {
  ...
  const [tokensUsed, setTokensUsed] = useState({}); // Track tokens per conversation
  const MAX_TOKENS_PER_ROOM = 100000;
  ...
  useEffect(() => {
    ...
    if (currentConversationId) {
      // Update token count for this conversation
      const newTokenCount = messages.reduce((sum, msg) => sum + countMessageTokens(msg), 0);
      setTokensUsed((prev) => ({
        ...prev,
        [currentConversationId]: newTokenCount,
      }));
  ...
  const handleSendMessage = async (e) => {
    ...
    // Check token limit
    const currentTokens = tokensUsed[currentConversationId] || 0;
    const messageTokens = countMessageTokens({ text: inputValue });
    
    if (!isWithinTokenLimit(currentTokens, messageTokens, MAX_TOKENS_PER_ROOM)) {
      setError(`Token limit exceeded. Max: ${MAX_TOKENS_PER_ROOM}, Used: ${currentTokens}, Message: ${messageTokens}`);
      return;
    }
    ...
    try {
      ...
      const response = await sendMessageToGrok(inputValue, messages, userLanguage, currentConversationId, selectedPersonality, abortController);
        ...
        <div className="token-section">
          ...
          <div className="token-info">
            <div className="token-stat">
              ...
              <span className="token-value">{tokensUsed[currentConversationId] || 0}</span>
            </div>
            <div className="token-stat">
              <span className="token-label">{userLanguage === 'id' ? 'Sisa' : 'Remaining'}:</span>
              <span className="token-value">{getRemainingTokens(MAX_TOKENS_PER_ROOM, tokensUsed[currentConversationId] || 0)}</span>
            </div>
            <div className="token-stat">
          ...
          <div className="token-bar-container">
            <div 
              ...
              style={{ width: `${(tokensUsed[currentConversationId] || 0) / MAX_TOKENS_PER_ROOM * 100}%` }}
            />
          </div>
        </div>
