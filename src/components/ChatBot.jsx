...
const ChatBot = () => {
  ...
  useEffect(() => {
    try {
      ...
      // Clear corrupted data dan buat fresh conversation
    ...
    if (conversations.length > 0) {
      try {
        ...
        if (publicConversations.length > 0) {
          localStorage.setItem('chatbot_conversations', JSON.stringify(publicConversations));
        }
      } catch (err) {
        console.error('Error saving conversations:', err);
        // Continue without saving jika localStorage penuh atau error
      }
    }
  }, [conversations]);
